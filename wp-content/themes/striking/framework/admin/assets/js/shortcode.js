var shortcode = {
	...
	generate:function(){
		...
		case 'portfolio':
			...
			var titleLinkable = shortcode.getVal('portfolio','titleLinkable');
			...
			if(titleLinkable===true){
				titleLinkable = ' titleLinkable="true"';
			} else {
				titleLinkable = ' titleLinkable="false"';
			}
			if(desc===true){
				desc = ' desc="true"';
			} else {
			...
			return '[portfolio'+column+height+nopaging+sortable+max+cat+ids+title+titleLinkable+desc+advanceDesc+more+moreText+group+lightboxTitle+order+orderby+']';
			break;
		case 'blog':
