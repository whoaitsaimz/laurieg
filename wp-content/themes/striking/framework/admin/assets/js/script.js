...
jQuery(document).ready( function($) {
	...
	jQuery(".range-input-wrap :range").rangeinput();//enable range input
	
	jQuery.tools.validator.addEffect("option", function(errors, event) {
		// add new ones
		jQuery.each(errors, function(index, error) {
			var input = error.input;
			input.addClass("invalid");
			var msg = input.next('.validator-error').empty();
			jQuery.each(error.messages, function(i, m) {
				jQuery("<span/>").html(m).appendTo(msg);			
			});
		});
	...
	// the effect does nothing when all inputs are valid	
		...
		inputs.removeClass("invalid").each(function() {
			jQuery(this).next('.validator-error').empty();
		});
	});
	jQuery(".validator-wrap :input").validator({effect:'option'});
	//mColorPicker setting
	jQuery.fn.mColorPicker.init.showLogo = false;
	jQuery.fn.mColorPicker.defaults.imageFolder = theme_admin_assets_uri + "/images/mColorPicker/";

	jQuery('.toggle-button:checkbox').each(function(){
		if(!jQuery(this).parents().is('.shortcode_wrap')){
	...
	jQuery('select.tri-toggle-button').each(function(){
		if(!jQuery(this).parents().is('.shortcode_wrap')){
			if(jQuery(this).parents('.postbox').is('.closed')){
				var button = jQuery(this);
