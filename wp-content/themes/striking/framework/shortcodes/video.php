...
HTML;
	...
	$output = "<div class=\"video_frame video-js-box\">".
	"<video class=\"video-js\" width=\"{$width}\" height=\"{$height}\" {$poster_attribute} controls {$preload_attribute} {$autoplay_attribute}>".
		$mp4_source.
		$webm_source.
		$ogg_source.
		"<object class=\"vjs-flash-fallback\" width=\"{$width}\" height=\"{$height}\" type=\"application/x-shockwave-flash\" data=\"http://releases.flowplayer.org/swf/flowplayer-3.2.5.swf\">".
...
function theme_video_youtube($atts, $content=null) {
	...
	if (!empty($clip_id)){
		return "<div class='video_frame'><iframe class='youtube' style='height:{$height}px;width:{$width}px' src='http://www.youtube.com/embed/{$clip_id}?autohide={$autohide}{$autoplay}&amp;controls={$controls}&amp;disablekb={$disablekb}&amp;fs={$fs}&amp;hd={$hd}&amp;loop={$loop}&amp;rel={$rel}&amp;showinfo={$showinfo}&amp;showsearch={$showsearch}&amp;wmode=transparent&amp;enablejsapi={$enablejsapi}' width='{$width}' height='{$height}' frameborder='0'></iframe></div>";
	}
}
